...
export async function handlePunch(tank: TankPacket, peer: Peer, base: BaseServer, world: World): Promise<void>  {
  ...
  if(itemMeta.breakHits == itemMeta.breakHits){
...
`2`
   ...
   // world.drop(peer, pos, pos, 112, randomNum)

    let gemsAdd = peer.data.gems + randomNum;

    peer.send(Variant.from("OnSetBux", parseInt(`${gemsAdd}`)));
      peer.data.gems = parseInt(`${gemsAdd}`);
      peer.saveToCache();
      peer.saveToDatabase();
    }

    if(itemMeta.isRayman){
      tankData.buildRange = 100
    }
